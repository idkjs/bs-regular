module Styles = {
  open Css
  let contextLayer = style(list{backgroundColor(#hex(Colors.primary475))})
}
...
let make = () => {
  ...
  <>
    <div ref={ReactDOM.Ref.domRef(divRef)}> {React.string("Context")} </div>
    {showLayer
      ? <>
          <ContextLayer position=ContextLayer.Top contextRef=divRef>
            {(~position as _) =>
              <div className=Styles.contextLayer> {React.string("ContextLayer")} </div>}
          </ContextLayer>
