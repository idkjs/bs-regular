...
let make = () => {
  ...
  let renderModal = React.useCallback1(
    () =>
      <Modal.Root>
        ...
        <Button onClick={_ => setShowModal(_ => false)}> {React.string("Hide")} </Button>
      </Modal.Root>,
    [count],
  )
  let onCloseRequest = React.useCallback0(() => setShowModal(_ => false))
  <>
    <Button onClick={_ => setShowModal(_ => true)}> {React.string("Show modal")} </Button>
