...
let layer = (~text, ~contextRef, ~position=?, ()) =>
  <ContextLayer ?position contextRef>
    {(~position as _) => <div className=Styles.layer> {React.string(text)} </div>}
  </ContextLayer>
...
let make = (~text, ~position=?, ~children) => {
  ...
  <>
    {children(~contextRef=ReactDOM.Ref.domRef(contextRef), ~onMouseEnter, ~onMouseLeave)}
    {showTooltip ? layer(layerProps(~contextRef, ~text, ~position?, ())) : React.null}
  </>
}
...
let container = (~text, ~position=?, ~children, ~className=?, ()) =>
  make(
    makeProps(
      ~text,
      ~position?,
      ~children=(~contextRef, ~onMouseEnter, ~onMouseLeave) =>
        <div ref=contextRef onMouseEnter onMouseLeave ?className> children </div>,
      (),
    ),
  )
