module Styles = {
  ...
  let option = style(list{
    ...
    lastChild(list{borderBottomStyle(#none)}),
  })
  let optionSelected = style(list{backgroundColor(#hex(Colors.primary490))})
  let optionFocused = style(list{textDecoration(#underline)})
}
  ...
  let make = (
    ...
    let onKeyPress = (e: Webapi.Dom.KeyboardEvent.t) => {
      ...
      | "Escape" =>
        onSelect(None)
      | "ArrowUp" =>
        setFocusedIndex(i => mod(i - 1 + numOptions, numOptions))
        Webapi.Dom.KeyboardEvent.preventDefault(e)
      | "ArrowDown" =>
        setFocusedIndex(i => mod(i + 1, numOptions))
        Webapi.Dom.KeyboardEvent.preventDefault(e)
      | "Enter" => onSelect(Belt.Array.get(options, focusedIndex))
      | key =>
        if String.length(key) == 1 {
          open Char
          let lowercaseKey = code(lowercase_ascii(String.get(key, 0)))
          if lowercaseKey >= code('a') && lowercaseKey <= code('z') {
            let keyFilterStr = keyFilter.current ++ String.lowercase_ascii(key)
            switch Js.Array.findIndex(
              (option: selectOption) =>
                Js.String.startsWith(keyFilterStr, Js.String.toLowerCase(option.label)),
              options,
            ) {
            | -1 => ()
            | firstMatchingOptionIndex => setFocusedIndex(_ => firstMatchingOptionIndex)
            }
            keyFilter.current = keyFilterStr
          }
          keyFilterTimeoutRef.current = Some(Js.Global.setTimeout(() => {
              keyFilter.current = ""
              keyFilterTimeoutRef.current = None
            }, 300))
        }
    ...
    React.useEffect0(() => {
      ...
      let onClick = e => {
        ...
        let layerDiv = Belt.Option.getExn(Js.Nullable.toOption(layerRef.current))
        let contextElement = Belt.Option.getExn(Js.Nullable.toOption(contextRef.current))
        let targetElement = EventTarget.unsafeAsElement(target)
        if (
          !Element.contains(targetElement, layerDiv) &&
    ...
    let scrollToElement = React.useCallback0(optionElement => {
      let layerDiv = Belt.Option.getExn(Js.Nullable.toOption(layerRef.current))
      open Webapi.Dom
    ...
    <ContextLayer position=ContextLayer.Bottom contextRef onKeyPress>
      {(~position as _) => {
        let inputElement = Belt.Option.getExn(Js.Nullable.toOption(contextRef.current))
        let style = ReactDOM.Style.make(
          ~width=string_of_int(Webapi.Dom.Element.clientWidth(inputElement)) ++ "px",
          (),
        )
        <div className=Styles.optionsLayer onMouseDown style ref={ReactDOM.Ref.domRef(layerRef)}>
          {React.array(Js.Array.mapi((option, i) => {
              let isSelected = switch selectedOption {
...
let make = (
  ...
  let onInputChange = React.useCallback3((e: ReactEvent.Form.t) => {
  ...
  let onBlur = React.useCallback1(_ => blurTimeout.current = Some(Js.Global.setTimeout(() => {
        switch selectedOption {
        | Some(selectedOption) => setTextValue(_ => selectedOption.label)
        | None => ()
        }
  ...
  <div onFocus={React.useCallback0(onFocus)} onBlur>
    {withTextInput
      ? <TextInput value=textValue onChange=onInputChange ?label ?placeholder ref=inputRef />
      : <TextInput.Static ?label tabIndex=0 onClick={_ => setShowOptions(_ => true)} ref=inputRef>
          {switch selectedOption {
    ...
    {showOptions
      ? Array.length(options) > 0
          ? <SelectOptions options selectedOption onSelect onMouseDown contextRef=inputRef />
          : React.null
      : React.null}
  </div>
}
