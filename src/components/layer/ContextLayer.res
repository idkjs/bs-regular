module Styles = {
  open Css
  let layer = style(list{position(#absolute)})
}
...
let getAnchor = (
  ...
  switch position {
  | Top => BottomLeft(scrollY +. top(contextRect), scrollX +. left(contextRect))
  | Bottom => TopLeft(scrollY +. bottom(contextRect), scrollX +. left(contextRect))
  | Left => TopRight(scrollY +. top(contextRect), scrollX +. left(contextRect))
  | Right => TopLeft(scrollY +. top(contextRect), scrollX +. right(contextRect))
  }
}
...
let make = (
  ...
  React.useLayoutEffect0(() => {
    let contextElement = switch Js.Nullable.toOption(contextRef.current) {
    | Some(element) => element
    | None => raise(RefHasNoElement)
    }
    let contextRect = Webapi.Dom.Element.getBoundingClientRect(contextElement)
  ...
  let outerStyle = Belt.Option.map(anchor, x =>
    switch x {
    | BottomLeft(y, x)
    | TopLeft(y, x)
    | TopRight(y, x) =>
      ReactDOM.Style.make(~top=Js.Float.toString(y) ++ "px", ~left=Js.Float.toString(x) ++ "px", ())
    }
  )
  let innerStyle = Belt.Option.map(anchor, x =>
    switch x {
  ...
  <Layer ?onKeyPress>
    <div ref={ReactDOM.Ref.domRef(divRef)} className=Styles.layer style=?outerStyle>
      <div className=Styles.layer style=?innerStyle> {children(~position)} </div>
    </div>
  </Layer>
}
